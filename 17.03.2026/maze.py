...
class PriorityFrontier:
    ...
    def remove(self):
        ...
        return self.frontier.pop(min(range(len(self.frontier)), key = lambda i : self.frontier[i][1]))[0]
...
class Maze:
    ...
    def solve(self):
        ...
        self.frontier.add(Node(state = self.start, parent = None, action = None, cost = 0), priority = self.heuristic(self.start))
        self.explored = set()
        ...
        while True:
            ...
            for action, new_state in self.neighbors(node.state):
                if not self.frontier.contains(new_state) and new_state not in self.explored:
                    self.frontier.add(Node(state = new_state, parent = node, action = action, cost = node.cost + 1), node.cost + self.heuristic(new_state) + 1)
